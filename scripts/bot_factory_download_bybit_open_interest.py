...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Download public Bybit V5 open-interest market data for Bot Factory "
            "local research screens. This command uses no API keys and does not "
    ...
    parser.add_argument(
        ...
        help="Bybit open-interest intervalTime.",
    )
    parser.add_argument(
        "--timerange",
        default=None,
...
def main() -> int:
    ...
    output = (
        Path(args.output)
        if args.output
        else default_bybit_open_interest_path(
            args.symbol,
