...
class OrderBookDepthStrategy(IStrategy):
    ...
    trailing_stop_positive_offset = 0.0055  # активация трейлинга при достижении 0.55% прибыли
            ...
            if current_entries < self.max_entry_position_adjustment:
                if current_profit <= dca_levels.get(current_entries, -999):
                    ...
                    if dca_amount < min_stake:
                        ...
                        return None
                    self.logger.info(
                        f"DCA докупка для {trade.pair} на уровне {current_profit * 100:.2f}%: {dca_amount:.2f} USDT"
                    )
                    return dca_amount

            # Частичная фиксация прибыли через единую систему
            if not getattr(trade, "has_open_orders", False) and self.partial_profit_enabled:
                # Проверяем все уровни частичной фиксации
                    ...
                    if current_profit >= level:
                        ...
                        if not getattr(trade, level_key, False):
                            ...
                            if exit_amount * current_rate >= min_stake:
                                # Отмечаем уровень как выполненный
    ...
    def custom_stake_amount(
        ...
        **kwargs,
        ...
        try:
            ...
            # Применяем логику ограничений с учетом баланса:
            # 1. Ограничиваем ATR-расчет балансом
            # 2. Применяем мин./макс. ограничения
            # 3. Финальная проверка min_stake

            # Сначала ограничиваем ATR-расчет балансом
            atr_based_size = min(atr_based_size, balance)

            # Затем применяем мин./макс. ограничения
            if atr_based_size < min_position:
                position_size = min_position
            elif atr_based_size > max_position:
                position_size = max_position
            else:
                position_size = atr_based_size

            # Финальная проверка: размер позиции не должен быть меньше min_stake
            position_size = max(position_size, min_stake)

            # Дополнительное логирование для отладки
            self.logger.info(
                f"Position size calculation for {pair}: "
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        try:
            ...
            if self._has_open_position(pair):
                ...
                # Устанавливаем сигнал на выход только если ROI достигнут
                if exit_long and roi_condition_met:
                    dataframe.loc[dataframe.index[-1], "exit_long"] = 1
                    self.logger.info(f"ИТОГОВЫЙ РЕЗУЛЬТАТ: СИГНАЛ НА ПРОДАЖУ! 📉")
                else:
                    dataframe.loc[dataframe.index[-1], "exit_long"] = 0
                    if exit_long and not roi_condition_met:
                        self.logger.info(f"ИТОГОВЫЙ РЕЗУЛЬТАТ: ВЫХОД ОТМЕНЕН (ROI не достигнут)")
                    elif not exit_long:
                        reason = "условия не выполнены"
            ...
            if self._get_param_value("atr_takeprofit_enabled"):
                ...
                if not dataframe.empty:
                    ...
                    if atr_value and not pd.isna(atr_value):
                        ...
                        # Рассчитываем целевой уровень прибыли
                        target_profit = (atr_value * tp_multiplier) / trade.open_rate

                        if current_profit >= target_profit and current_profit >= tp_min_pct:
                            self.logger.info(
                                f"ATR Take-Profit для {pair}: прибыль {current_profit * 100:.2f}% >= "
                                f"целевой {target_profit * 100:.2f}% (ATR: {atr_value:.8f})"
                            )
    ...
    def _calculate_rsi_tradingview(self, prices, period=14):
        """
        Расчет RSI в стиле TradingView
        ...
        try:
            ...
            # Рассчитываем средние значения с использованием экспоненциального скользящего среднего
            ...
            # Возвращаем NaN в случае ошибки
