...
STRATEGY_PARAMS = {
    "timeframe": "5m",
    "order_book_depth": 50,
    "min_cluster_size": 0.05,  # Снижено с 0.1 для более частых входов,
    "cluster_price_threshold": 0.0005,
    "min_orders_in_cluster": 2,
    "min_volume_ratio": 0.8,  # Снижено с 1.5 для соответствия текущим объемам. Используется для проверки volume_ratio в условиях входа/выхода
    "volume_threshold": 2.0,  # Используется для фильтрации кластеров по объему
    "cluster_price_distance": 0.002,  # Увеличено с 0.001 для более широкого диапазона. Максимальное расстояние до кластера (0.1%)
    "stoploss": -0.015,  # Снижено с -0.02 для меньших потерь,
    "minimal_roi": {
        "0": 0.03,  # Снижено с 0.05 для более быстрого выхода
        "15": 0.02,  # Добавлен промежуточный уровень
        "30": 0.015,  # Снижено с 0.025
        "60": 0.01,  # Снижено с 0.015
        "120": 0.008,  # Снижено с 0.01
    },
    # Фильтры для входа и выхода
    # RSI фильтр для входа и выхода
    "rsi_filter_enabled": True,
    "rsi_period": 14,  # было 14
    "rsi_buy_threshold": 49,  # /45/ Увеличено с 35 для более частых входов
    "rsi_sell_threshold": 70,  # Снижено с 75 для более быстрого выхода
    # EMA фильтр тренда
    ...
    "atr_period": 14,
    "atr_min_volatility_pct": 0.09,  # /0.15/ Снижено с 0.3 для соответствия текущей волатильности, (было 0.5 Минимальная волатильность в % от цены )
    # ATR для Stop-Loss и Take-Profit
    "atr_stoploss_enabled": True,
    "atr_stoploss_multiplier": 1.5,  # Снижено с 2.0 для более близкого стоп-лосса. Множитель ATR для стоп-лосса
    "atr_takeprofit_enabled": True,
    "atr_takeprofit_multiplier": 2.5,  # Снижено с 3.0 для более быстрого тейк-профита. Множитель ATR для тейк-профита
    "atr_takeprofit_min_pct": 0.9,  # /0.8/ Снижено с 1.0 для более быстрого выхода. Минимальный % прибыли для ATR тейк-профита
    # === DCA (Докупание при просадке) ===
    "dca_enabled": True,  # Включатель/выключатель DCA
    "max_dca_entries": 2,  # Максимум 2 дополнительные закупки
    "min_dca_time": 2,  # Минимальный промежуток между докупками (в минутах)
    "dca_levels": {0: -0.005, 1: -0.01},  # Уровни просадки для докупки (-0.5%, -1%)
    "dca_multiplier": 1.5,  # Множитель размера позиции для каждой докупки
    # === Частичная фиксация прибыли ===
    "partial_profit_enabled": True,  # Включатель/выключатель частичной фиксации
    "partial_profit_levels": {0.005: 0.3, 0.01: 0.3},  # Уровни прибыли и % позиции для фиксации
    "partial_profit_min_amount": 0.001,  # Минимальный размер частичной фиксации
    # === Динамический расчет размера позиции ===
    "dynamic_position_enabled": True,  # Включатель/выключатель динамического расчета
    "risk_per_trade": 0.03,  # 3% риска на сделку
    "atr_position_multiplier": 2.0,  # Множитель ATR для расчета размера позиции
    "max_position_size_pct": 10,  # Максимальный размер позиции в % от указанного в config_OrderBookDepth.json stake_amount
    "min_position_size_pct": 5,  # Минимальный размер позиции в % от указанного в config_OrderBookDepth.json stake_amount
}
...
DEFAULT_SETTINGS = {
    ...
    # === DCA параметры ===
    "dca_enabled": STRATEGY_PARAMS["dca_enabled"],
    "max_dca_entries": STRATEGY_PARAMS["max_dca_entries"],
    "min_dca_time": STRATEGY_PARAMS["min_dca_time"],
    "dca_levels": STRATEGY_PARAMS["dca_levels"],
    "dca_multiplier": STRATEGY_PARAMS["dca_multiplier"],
    # === Частичная фиксация прибыли ===
    "partial_profit_enabled": STRATEGY_PARAMS["partial_profit_enabled"],
    "partial_profit_levels": STRATEGY_PARAMS["partial_profit_levels"],
    "partial_profit_min_amount": STRATEGY_PARAMS["partial_profit_min_amount"],
    # === Динамический расчет размера позиции ===
    "dynamic_position_enabled": STRATEGY_PARAMS["dynamic_position_enabled"],
    "risk_per_trade": STRATEGY_PARAMS["risk_per_trade"],
    "atr_position_multiplier": STRATEGY_PARAMS["atr_position_multiplier"],
    "max_position_size_pct": STRATEGY_PARAMS["max_position_size_pct"],
    "min_position_size_pct": STRATEGY_PARAMS["min_position_size_pct"],
...
class OrderBookDepthStrategy(IStrategy):
    ...
    # === DCA, частичная фиксация прибыли и динамический размер позиции ===
    # Эти параметры теперь берутся из STRATEGY_PARAMS через _get_param_value()
    # Старые значения оставлены для совместимости, но не используются
    min_dca_time = 2  # Минимальный промежуток между докупками (в минутах)
    ...
    def __init__(self, config: dict) -> None:
        ...
        # Добавляем кэш для стаканов
        self._orderbook_cache = {}
        self._cache_timeout = 5  # секунды

        # Добавить в __init__:
        self._last_log_time = {}
        self._log_interval = 10  # секунды
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        try:
            ...
            # Проверяем доступность exchange
            if not hasattr(self, "dp") or not hasattr(self.dp, "_exchange"):
                self.logger.warning(f"Exchange not available for {pair}")
                return self._add_default_indicators(dataframe)

            # Получаем стакан с кэшированием
            try:
                orderbook = self._get_cached_orderbook(
                    pair, self._get_param_value("order_book_depth")
                )
            except Exception as e:
                self.logger.warning(f"Failed to get orderbook for {pair}: {e}")
                return self._add_default_indicators(dataframe)

            # Логируем информацию о стакане
            logger.debug(f"Order book depth: {self._get_param_value('order_book_depth')}")
            ...
            self.logger.error(
                f"Critical error in populate_indicators for {metadata.get('pair')}: {e}"
            )
            return self._add_default_indicators(dataframe)

    def _add_default_indicators(self, dataframe):
        """Добавляет базовые индикаторы при недоступности стакана"""
        dataframe["rsi"] = ta.RSI(dataframe, timeperiod=self._get_param_value("rsi_period"))
        dataframe["ema_slow"] = ta.EMA(
            dataframe, timeperiod=self._get_param_value("ema_slow_period")
        )
        ...
        # Устанавливаем нулевые значения для кластеров
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        try:
            ...
            if self._has_open_position(pair):
                ...
                # Проверяем ROI только для прибыльных сделок
                if current_profit > 0:
                    roi_condition_met = self._check_roi_condition(pair, current_price)
                else:
                    roi_condition_met = True  # Для убыточных сделок разрешаем выход
    ...
    def _get_cached_orderbook(self, pair, depth):
        current_time = time.time()
        cache_key = f"{pair}_{depth}"

        if cache_key in self._orderbook_cache:
            cached_data, cache_time = self._orderbook_cache[cache_key]
            if current_time - cache_time < self._cache_timeout:
                return cached_data

        # Получаем новый стакан
        orderbook = self.dp._exchange.fetch_l2_order_book(pair, depth)
        self._orderbook_cache[cache_key] = (orderbook, current_time)
        return orderbook

    def _log_if_needed(self, pair, message, level="info"):
        current_time = time.time()
        if (
            pair not in self._last_log_time
            or current_time - self._last_log_time[pair] > self._log_interval
        ):
            getattr(self.logger, level)(message)
            self._last_log_time[pair] = current_time
