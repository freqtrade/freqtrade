...
class OrderBookDepthStrategy(IStrategy):
    ...
    def custom_stake_amount(
        ...
        try:
            ...
            if atr <= 0:
                self.logger.warning(f"Invalid ATR for {pair}: {atr}")
                return min_stake
            # Рассчитываем размер позиции на основе ATR
            available_balance = max_stake  # Можно доработать под реальный баланс
            ...
            if position_size < min_stake:
                self.logger.warning(
                    f"Calculated position size {position_size} too small. Using min stake {min_stake}"
                )
                return min_stake
            self.logger.info(f"Dynamic position sizing for {pair}: ATR={atr}, size={position_size}")
            return position_size
        except Exception as e:
            self.logger.error(f"Error in custom_stake_amount for {pair}: {e}")
            return min_stake
