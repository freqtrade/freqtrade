...
class OrderBookDepthStrategy(IStrategy):
    ...
    max_position_size_pct = 95  # % от stake_amount из конфига
    min_position_size_pct = 10  # % от stake_amount из конфига
    ...
    partial_profit_levels = {
        0.015: 0.3,  # При 1.5% прибыли закрываем 30% позиции
        0.03: 0.3,  # При 3% прибыли закрываем еще 30%
        0.05: 0.4,  # При 5% прибыли закрываем оставшиеся 40%
    }
    ...
    def adjust_trade_position(
        ...
        """
        try:
            # DCA уровни (можно настроить)
            dca_levels = {0: -0.005, 1: -0.01}  # -0.5%, -1%
            current_entries = getattr(trade, "nr_of_successful_entries", 0)
            # Минимальные требования биржи (заглушка, если нужно - доработать)
            ...
            if current_entries < self.max_entry_position_adjustment:
                if current_profit <= dca_levels.get(current_entries, -999):
                    ...
                    if dca_amount < min_stake_required:
                        self.logger.warning(
                            f"DCA amount {dca_amount} too small. Min required: {min_stake_required}"
                        )
                        return None
                    return dca_amount
            # Частичная фиксация прибыли
            if not getattr(trade, "has_open_orders", False):
                if current_profit >= 0.01:
                    exit_amount = trade.amount * 0.3
                    if exit_amount * current_rate < min_stake or exit_amount < min_stake:
                        self.logger.warning(
                            f"Exit amount {exit_amount} too small. Min: {min_stake}"
                        )
                        return None
                    return -exit_amount
    ...
    def custom_stake_amount(
        ...
        **kwargs,
        ...
        try:
            ...
            if dataframe.empty or len(dataframe) < self.atr_period:
            ...
            min_position = config_stake_amount * (self.min_position_size_pct / 100)
            max_position = config_stake_amount * (self.max_position_size_pct / 100)

            # Рассчитываем размер позиции на основе ATR и управления риском
            risk_amount = balance * self.risk_per_trade
            atr_based_size = (risk_amount / (atr * self.atr_position_multiplier)) * current_rate

            # Применяем ограничения: min_position <= size <= min(max_position, max_stake, balance)
            max_allowed = min(max_position, max_stake, balance)
            position_size = max(min(atr_based_size, max_allowed), min_position)

            # Дополнительное логирование для отладки
            self.logger.debug(
                f"Position size calculation - "
                f"ATR: {atr:.8f}, ATR-based: {atr_based_size:.2f}, "
                f"Min: {min_position:.2f}, Max allowed: {max_allowed:.2f}, "
                f"Final: {position_size:.2f}"
            )

            self.logger.info(
                f"Dynamic position sizing for {pair}: "
                f"Balance={balance:.2f}, ATR={atr:.8f}, "
                f"Risk={self.risk_per_trade * 100}%, Size={position_size:.8f}"
            )
    ...
    def _log_signals(self, pair: str, dataframe: DataFrame, ob_analysis: dict) -> None:
        ...
        try:
            ...
            status_msg = [
                f"[{pair}] Цена: {last_row['close']:.2f} | Позиция: {'ДА' if has_position else 'НЕТ'} | "
    ...
    def custom_exit(
        ...
        **kwargs,
        ...
        """
        Частичная фиксация прибыли на основе заданных уровней

        Args:
            pair: Торговая пара
            trade: Объект текущей сделки
            current_time: Текущее время
            current_rate: Текущая цена
            current_profit: Текущая прибыль в %

        Returns:
            Optional[str]: Тег выхода или None, если выход не требуется
        """
        try:
            if not self.partial_profit_enabled or not self.partial_profit_levels:
                return None

            # Проверяем, есть ли открытые ордера на выход
            if trade.has_open_orders:
                return None

            # Получаем все уровни, которые еще не были достигнуты
            levels_to_check = {
                k: v
                for k, v in self.partial_profit_levels.items()
                if k > trade.take_profit_pct and current_profit >= k
            }
            ...
            sorted_levels = sorted(levels_to_check.items())

            for level, exit_percent in sorted_levels:
                # Проверяем, достигнут ли уровень
                if current_profit >= level:
                    # Рассчитываем количество для закрытия
                    amount_to_close = trade.amount * exit_percent

                    # Проверяем минимальный размер ордера
                    min_notional = (
                        self.dp._exchange.markets[pair].get("limits", {}).get("cost", {}).get("min")
                    )
                    if min_notional and (amount_to_close * current_rate) < min_notional:
                        self.logger.info(
                            f"Skipping partial exit at {level * 100:.2f}%: amount too small"
                        )
                        continue

                    # Создаем ордер на частичное закрытие
                    self.logger.info(
                        f"Частичная фиксация прибыли для {pair} на уровне {level * 100:.2f}%: "
                        f"закрываем {exit_percent * 100:.0f}% позиции ({amount_to_close:.8f} {pair.split('/')[0]})"
                    )

                    return {
                        "exit_tag": f"partial_profit_{int(level * 100)}",
