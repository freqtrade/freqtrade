...
class Strategy_DCA(IStrategy):
    ...
    minimal_roi = {
        "0": 0.02
    }  # ROI = 2% (для безубыточного скальпинга на Binance min ROI >= 0.55% )
    stoploss = -0.0075  # SL = 0.75%
    timeframe = "5m"  # 1-минутный таймфрейм для более частой торговли
    ...
    # Параметры для настройки сигнала дисбаланса книги ордеров
    order_book_depth: int = 10
    use_imbalance_filter: bool = True  # Выключатель использования сигнала дисбаланса
    use_volume_filter: bool = True  # Выключатель использования объемного сигнала
    use_rsi_filter: bool = True  # Выключатель использования RSI
    use_ema_filter: bool = True  # Выключатель использования EMA

    # Параметры для анализа объема
    volume_ma_period = 20  # к-во свечей для расчета скользящей средней объема

    # Параметры для обычного режима (без всплеска)
    normal_buy_imbalance_threshold: float = 1.2  # Порог дисбаланса для обычной покупки
    normal_buy_volume_threshold: float = 0.1  # Порог объема для обычной покупки (уменьшен с 1.0)
    normal_buy_rsi_threshold: float = 40  # Порог RSI для обычной покупки

    normal_sell_imbalance_threshold: float = 0.5  # Порог дисбаланса для обычной продажи
    normal_sell_volume_threshold: float = 0.1  # Порог объема для обычной продажи
    normal_sell_rsi_threshold: float = 65  # Порог RSI для обычной продажи

    # Параметры для режима всплеска
    surge_buy_imbalance_threshold: float = 2.0  # Порог дисбаланса при всплеске покупок
    surge_buy_volume_threshold: float = 0.25  # Порог объема при всплеске покупок (уменьшен с 3.0)
    surge_buy_rsi_threshold: float = 30  # Порог RSI при всплеске покупок

    surge_sell_imbalance_threshold: float = 0.4  # Порог дисбаланса при всплеске продаж
    surge_sell_volume_threshold: float = 0.25  # Порог объема при всплеске продаж
    surge_sell_rsi_threshold: float = 70  # Порог RSI при всплеске продаж

    # Параметры для альтернативных условий входа (всплеск)
    use_alternative_entry: bool = False  # Включаем альтернативные условия
    rsi_oversold_threshold: float = 35  # Порог перепроданности RSI (увеличен с 35)
    strong_imbalance_multiplier: float = 2.0  # Порог сильного дисбаланса (уменьшен с 2.0)
    volume_surge_threshold: float = 2.5  # Порог всплеска объема (уменьшен с 2.5)

    # Параметры для анализа тренда (EMA)
    trend_ema_fast = 9  # EMA быстрый период
    trend_ema_slow = 21  # EMA медленный период
    trend_ema_signal = 20  # EMA сигнал период
    trend_strength_threshold: float = (
        0.000001  # Сила тренда порог для покупки (уменьшен с 0.000005)
    )

    ema_condition_weights = {  # Веса для условий EMA (всего должно быть 1.0)
        "ema_fast_slow": 0.5,  # Вес условия быстрая/медленная EMA (увеличен с 0.5)
        "ema_slow_signal": 0.2,  # Отключаем условие медленная/сигнальная EMA
        "trend_strength": 0.3,  # Вес условия силы тренда
    }

    condition_ema_threshold = 0.3  # Порог для условий EMA (уменьшен с 0.3)

    # Дополнительные проверки для специфических пар
    _pair_specific_thresholds = {
        "BTC/USDT": {
            "normal_buy_imbalance_threshold": 2.3,  # Порог дисбаланса для обычной покупки
            "normal_buy_volume_threshold": 0.16,  # Порог объема для обычной покупки
            "normal_buy_rsi_threshold": 36,  # Порог RSI для обычной покупки
        },
        "ETH/USDT": {
            "normal_buy_imbalance_threshold": 1.29,  # Порог дисбаланса для обычной покупки
            "normal_buy_volume_threshold": 0.03,  # Порог объема для обычной покупки
            "normal_buy_rsi_threshold": 35,  # Порог RSI для обычной покупки
        },
        "BNB/USDT": {
            "normal_buy_imbalance_threshold": 7.2,  # Порог дисбаланса для обычной покупки
            "normal_buy_volume_threshold": 0.13,  # Порог объема для обычной покупки
            "normal_buy_rsi_threshold": 40,  # Порог RSI для обычной покупки
        },
        "DOT/USDT": {
            "normal_buy_imbalance_threshold": 1.5,  # Порог дисбаланса для обычной покупки
            "normal_buy_volume_threshold": 4.1,  # Порог объема для обычной покупки
            "normal_buy_rsi_threshold": 39.5,  # Порог RSI для обычной покупки
        },
        "SOL/USDT": {
            "normal_buy_imbalance_threshold": 1.45,  # Порог дисбаланса для обычной покупки
            "normal_buy_volume_threshold": 0.4,  # Порог объема для обычной покупки
            "normal_buy_rsi_threshold": 35,  # Порог RSI для обычной покупки
        },
    }
    ...
    trailing_stop = True
    trailing_stop_positive = 0.004  # отставание трейлинг-стопа на 0.4% от цены
    trailing_stop_positive_offset = 0.0045  # смещение  0.45%
    trailing_only_offset_is_reached = False  # Активация только после достижения смещения
    ...
    # Специфичные настройки кулдауна для разных пар (отключение пары на определенное время после закрытия с убытком)
    _pair_specific_cooldown = {
        "BTC/USDT": {
            "cooldown_period": 120,  # 2 часа для BTC
        },
        "ETH/USDT": {
            "cooldown_period": 60,  # 1 час для ETH
        },
        "BNB/USDT": {
            "cooldown_period": 90,  # 1.5 часа для BNB
        },
        "DOT/USDT": {
            "cooldown_period": 45,  # 45 минут для DOT
        },
        "SOL/USDT": {
            "cooldown_period": 60,  # 1 час для SOL
        },
    }
    ...
    risk_per_trade = 0.02  # 2% риска на сделку (было 0.02)
    ...
    def get_pair_specific_cooldown(self, pair: str) -> int:
        ...
        if pair in self._pair_specific_cooldown:
            return self._pair_specific_cooldown[pair].get("cooldown_period", self.cooldown_period)
    ...
    def populate_buy_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        normal_condition_rsi = rsi_val < self.normal_buy_rsi_threshold
        normal_condition_ema = (
            ema_fast > ema_slow
            and ema_slow > ema_signal
        ...
        if self.use_rsi_filter:
            logger.info(
                f"  - RSI: {rsi_val} "
                f"(threshold: {self.normal_buy_rsi_threshold}, "
        ...
        if self.use_ema_filter:
            ...
            if trend_strength > self.trend_strength_threshold:
                ema_score += self.ema_condition_weights["trend_strength"]
            ...
            logger.info(
                f"    3. Trend Strength > threshold: {trend_strength > self.trend_strength_threshold} ({trend_strength} > {self.trend_strength_threshold})"
            )
        ...
        if final_condition:
            ...
            if self.use_rsi_filter and not normal_condition_rsi:
                logger.info(f"  - RSI too high: {rsi_val} >= {self.normal_buy_rsi_threshold}")
    ...
    def populate_sell_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        normal_condition_ema = (
            ema_fast < ema_slow and trend_strength < -self.trend_strength_threshold
        )
    ...
    def custom_sell(
        ...
        **kwargs,
        ...
        logger.info(f"  - Threshold for take profit: 0.9")
        logger.info(
        ...
        if (
            imbalance_ratio <= threshold * 0.5 and current_profit > 0.015
        ):  # Увеличиваем до 1.5%(было 1%)
        ...
        if current_profit > 0.02 and imbalance_ratio < 0.9:  # Увеличиваем до 2%(было 1%)
    ...
    def custom_stake_amount(
        ...
        **kwargs,
        ...
        try:
            ...
            # Получаем минимальные требования с биржи
            try:
                ...
                logger.info(f"Exchange minimum requirements for {pair}:")
                logger.info(f"  - Min amount: {min_amount}")
                logger.info(f"  - Min cost: {min_cost} USDT")
                logger.info(f"  - Min stake required: {min_stake_required} USDT")
            except Exception as e:
                logger.error(f"Error getting exchange requirements for {pair}: {str(e)}")
                min_stake_required = min_stake  # Используем min_stake как запасной вариант
            ...
            return 0  # В случае ошибки возвращаем 0, что приведет к пропуску сделки
