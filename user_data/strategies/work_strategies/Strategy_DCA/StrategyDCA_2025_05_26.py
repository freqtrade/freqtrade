...
from freqtrade.strategy.interface import IStrategy  # Order Book Imbalance Scalping Strategy
...
print("StrategyDCA module loaded")
logger = logging.getLogger("StrategyDCA")
...
class StrategyDCA(IStrategy):
    ...
    # Настройки стратегии
    minimal_roi = {"0": 0.01}  # 0.55% прибыли достаточно для скальпинга
    stoploss = -0.005  # Стоп-лосс 1.5%
    ...
    # Параметры для настройки сигнала дизбаланса книги ордеров
    order_book_depth: int = 10
    use_imbalance_filter: bool = True  # Выключатель использования сигнала дисбаланса
    buy_imbalance_threshold: float = 1.3  # дизбаланс порог поукупки (1.3 - 1.4)
    sell_imbalance_threshold: float = 0.4  # дизбаланс порог продажи (0.4 - 0.5)
    ...
    volume_threshold: float = 0.8  # объем порог поукупки (0.8 - 0.9)
    volume_sell_threshold: float = 1.1  # объем порог продажи (1.0 - 1.2)

    # Дополнительные индикаторы для фильтрации сигналов
    use_rsi_filter: bool = True
    rsi_buy_threshold: float = 35  # RSI порог поукупки (35 - 40)
    rsi_sell_threshold: float = 60  # RSI порог продажи (60 - 65)

    # Параметры для анализа тренда (EMA)
    use_ema_filter: bool = True
    trend_ema_fast = 5  # EMA быстрый период (уменьшаем с 7 до 5 для еще более быстрой реакции)
    trend_ema_slow = 10  # EMA медленный период (уменьшаем с 14 до 10 для меньшего лага)
    trend_ema_signal = 20  # EMA сигнал период (уменьшаем с 30 до 20 для более быстрой реакции)
    trend_strength_threshold: float = (
        0.00005  # Сила тренда порог для покупки (уменьшаем с 0.0001 до 0.00005)
    )

    ema_condition_weights = {  # Веса для условий EMA (всего должно быть 1.0)
        "ema_fast_slow": 0.4,  # Вес условия быстрая/медленная EMA
        "ema_slow_signal": 0.25,  # Вес условия медленная/сигнальная EMA
        "trend_strength": 0.35,  # Вес условия силы тренда
    }

    condition_ema_threshold = 0.7  # Порог для условий EMA (71% условий должны быть выполнены)
    ...
    trailing_stop = True
    trailing_stop_positive = 0.005  # Увеличиваем активацию трейлинг-стопа до 0.5% прибыли
    trailing_stop_positive_offset = 0.007  # Увеличиваем смещение до 0.7%
    ...
    min_dca_time = 2  # Минимальный промежуток между докупками (в минутах) (3)
    # End DCA
    ...
    def __init__(self, config: dict):
        ...
        self.timeframe = config.get("timeframe", "5m")  # 5m по умолчанию при отсутствии в конфиге
    ...
    def populate_buy_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Условия
        condition_imbalance = (
            buy_imbalance > self.buy_imbalance_threshold if self.use_imbalance_filter else True
        )
        condition_volume = volume_ratio > self.volume_threshold if self.use_volume_filter else True
        condition_rsi = rsi_val < self.rsi_buy_threshold if self.use_rsi_filter else True
        condition_ema_fast = ema_fast > ema_slow * 0.999 if self.use_ema_filter else True
        condition_ema_slow = ema_slow > ema_signal * 0.999 if self.use_ema_filter else True
        condition_trend = (
            trend_strength > self.trend_strength_threshold if self.use_ema_filter else True
        )

        # Логируем значения индикаторов независимо от состояния фильтров
        logger.info(
            f"  - Imbalance ratio: {buy_imbalance} "
            f"(threshold: {self.buy_imbalance_threshold}, "
            f"condition: {condition_imbalance}, "
            f"filter: {'enabled' if self.use_imbalance_filter else 'disabled'})"
        )
        logger.info(
            f"  - RSI: {rsi_val} "
            f"(threshold: {self.rsi_buy_threshold}, "
            f"condition: {condition_rsi}, "
            f"filter: {'enabled' if self.use_rsi_filter else 'disabled'})"
        )
        logger.info(
            f"  - Volume ratio: {volume_ratio} "
            f"(threshold: {self.volume_threshold}, "
            f"condition: {condition_volume}, "
            f"filter: {'enabled' if self.use_volume_filter else 'disabled'})"
        )
        logger.info(
            f"  - Trend strength: {trend_strength} "
            f"(threshold: {self.trend_strength_threshold}, "
            f"condition: {condition_trend}, "
            f"filter: {'enabled' if self.use_ema_filter else 'disabled'})"
        )
        logger.info(f"  - EMA Fast: {ema_fast}")
        logger.info(f"  - EMA Slow: {ema_slow}")
        logger.info(
            f"  - EMA condition: {ema_fast} > {ema_slow} (filter: {'enabled' if self.use_ema_filter else 'disabled'})"
        )

        # Добавляем логирование для EMA условий
        if self.use_ema_filter:
            logger.info(f"  - EMA Fast/Slow ratio: {ema_fast / ema_slow:.6f} (threshold: 0.999)")
            logger.info(
                f"  - EMA Slow/Signal ratio: {ema_slow / ema_signal:.6f} (threshold: 0.999)"
            )
            logger.info(
                f"  - Trend strength: {trend_strength:.6f} (threshold: {self.trend_strength_threshold})"
            )

        # Условия EMA с меньшими требованиями
        ema_fast_slow_ratio = ema_fast / ema_slow
        ema_slow_signal_ratio = ema_slow / ema_signal

        # Более мягкие условия с допусками
        condition_ema_fast = (
            ema_fast_slow_ratio > 0.998 if self.use_ema_filter else True
        )  # Уменьшаем требование до 0.2%
        condition_ema_slow = (
            ema_slow_signal_ratio > 0.998 if self.use_ema_filter else True
        )  # Уменьшаем требование до 0.2%
        condition_trend = (
            trend_strength > self.trend_strength_threshold if self.use_ema_filter else True
        )

        # Взвешенная оценка условий EMA
        if self.use_ema_filter:
            ema_score = 0.0
            if condition_ema_fast:
                ema_score += self.ema_condition_weights["ema_fast_slow"]
            if condition_ema_slow:
                ema_score += self.ema_condition_weights["ema_slow_signal"]
            if condition_trend:
                ema_score += self.ema_condition_weights["trend_strength"]

            # Требуем выполнения условий EMA согласно порогу
            condition_ema = ema_score >= self.condition_ema_threshold
        else:
            condition_ema = True

        # Добавляем подробное логирование для EMA условий
        if self.use_ema_filter:
            logger.info(f"  - EMA Fast/Slow ratio: {ema_fast_slow_ratio:.6f} (threshold: 0.998)")
            logger.info(
                f"  - EMA Slow/Signal ratio: {ema_slow_signal_ratio:.6f} (threshold: 0.998)"
            )
            logger.info(
                f"  - Trend strength: {trend_strength:.6f} (threshold: {self.trend_strength_threshold})"
            )
            ...
            logger.info(f"  - EMA conditions met: {condition_ema}")

        conditions = [
            condition_imbalance,
            condition_volume,
            condition_rsi,
            condition_ema,  # Используем взвешенное условие вместо отдельных
            # Убираем отдельные условия EMA из списка
        ]
    ...
    def populate_sell_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Условия
        condition_imbalance = (
            sell_imbalance < self.sell_imbalance_threshold if self.use_imbalance_filter else True
        )
        ...
        condition_volume = (
            volume_ratio > self.volume_sell_threshold if self.use_volume_filter else True
        )
        condition_trend = (
            trend_strength < -self.trend_strength_threshold if self.use_ema_filter else True
        )
        ...
        # Логируем значения индикаторов независимо от состояния фильтров
        logger.info(
            f"  - Imbalance ratio: {sell_imbalance} "
            f"(threshold: {self.sell_imbalance_threshold}, "
            f"condition: {condition_imbalance}, "
            f"filter: {'enabled' if self.use_imbalance_filter else 'disabled'})"
        )
        logger.info(
            f"  - RSI: {rsi_val} "
            f"(threshold: {self.rsi_sell_threshold}, "
            f"condition: {condition_rsi}, "
            f"filter: {'enabled' if self.use_rsi_filter else 'disabled'})"
        )
        logger.info(
            f"  - Volume ratio: {volume_ratio} "
            f"(threshold: {self.volume_sell_threshold}, "
            f"condition: {condition_volume}, "
            f"filter: {'enabled' if self.use_volume_filter else 'disabled'})"
        )
        logger.info(f"  - EMA Fast: {ema_fast}")
        logger.info(f"  - EMA Slow: {ema_slow}")
        logger.info(
            f"  - EMA condition: {ema_fast} < {ema_slow * 1.001} (filter: {'enabled' if self.use_ema_filter else 'disabled'})"
        )
        logger.info(
            f"  - Trend strength: {trend_strength} "
            f"(threshold: {self.trend_strength_threshold}, "
        ...
        # Добавляем логирование для EMA условий
        if self.use_ema_filter:
            logger.info(f"  - EMA Fast/Slow ratio: {ema_fast / ema_slow:.6f} (threshold: 1.001)")
            logger.info(
                f"  - Trend strength: {trend_strength:.6f} (threshold: {-self.trend_strength_threshold})"
            )
        ...
        # Более мягкие условия с допусками
        condition_ema = (
            ema_fast_slow_ratio < 1.002 if self.use_ema_filter else True
        )  # Увеличиваем допуск до 0.2%
        condition_trend = (
            trend_strength < -self.trend_strength_threshold if self.use_ema_filter else True
        )

        # Взвешенная оценка условий EMA для продажи
        if self.use_ema_filter:
            ema_score = 0.0
            if condition_ema:
                ema_score += self.ema_condition_weights["ema_fast_slow"]
        ...
        # Добавляем подробное логирование для EMA условий
        if self.use_ema_filter:
            logger.info(f"  - EMA Fast/Slow ratio: {ema_fast_slow_ratio:.6f} (threshold: 1.002)")
            logger.info(
                f"  - Trend strength: {trend_strength:.6f} (threshold: {-self.trend_strength_threshold})"
            )
            logger.info(
        ...
        conditions = [
            condition_imbalance,
            condition_rsi,
            condition_ema,  # Используем взвешенное условие
            condition_volume,
        ]
        final_condition = all(conditions)
    ...
    def custom_sell(
        ...
        **kwargs,
        ...
        logger.info(f"  - Threshold for strong bearish: {self.sell_imbalance_threshold * 0.5}")
        ...
        if (
            imbalance_ratio <= self.sell_imbalance_threshold * 0.5 and current_profit > 0.01
        ):  # Увеличиваем до 1%
        ...
        if current_profit > 0.01 and imbalance_ratio < 0.9:  # Увеличиваем до 1%
