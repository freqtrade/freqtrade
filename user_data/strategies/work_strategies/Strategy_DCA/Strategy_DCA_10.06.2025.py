...
class Strategy_DCA(IStrategy):
    ...
    # Настройки стратегии
    minimal_roi = {
        "0": 0.01,  # ROI = 1% (уменьшен для более быстрой фиксации прибыли)
        "30": 0.015,  # ROI = 1.5% после 30 минут
        "60": 0.02,  # ROI = 2% после 60 минут
    }
    stoploss = -0.003  # SL = 0.3% (уменьшен для более быстрого выхода из убыточных позиций)
    timeframe = "1m"  # 1-минутный таймфрейм для более частой торговли

    # Добавляем отсутствующий атрибут trend_strength_threshold
    ...
    normal_buy_imbalance_threshold: float = 1.8  # Увеличен для более сильных сигналов
    normal_buy_volume_threshold: float = 0.2  # Увеличен для более сильных сигналов
    normal_buy_rsi_threshold: float = 40  # Уменьшен для более ранних входов

    normal_sell_imbalance_threshold: float = 0.7  # Увеличен для более консервативных выходов
    normal_sell_volume_threshold: float = 0.2  # Увеличен для более сильных сигналов
    normal_sell_rsi_threshold: float = 65  # Увеличен для более поздних выходов

    # Параметры для режима всплеска
    surge_buy_imbalance_threshold: float = 2.2  # Уменьшен для более частых входов
    surge_buy_volume_threshold: float = 0.25  # Уменьшен для более частых входов
    surge_buy_rsi_threshold: float = 35  # Оставлен без изменений

    surge_sell_imbalance_threshold: float = 0.4  # Увеличен для более консервативных выходов
    surge_sell_volume_threshold: float = 0.25  # Уменьшен для более частых выходов
    surge_sell_rsi_threshold: float = 70  # Увеличен для более поздних выходов

    # Параметры для анализа тренда (EMA)
    trend_ema_fast = 5  # оставить как есть
    trend_ema_slow = 13  # оставить как есть
    trend_ema_signal = 6  # было 8 - уменьшено для более быстрой реакции

    # Веса для условий EMA
    ema_condition_weights = {
        "ema_fast_slow": 0.4,  # было 0.5
        "ema_slow_signal": 0.3,  # было 0.2
        "trend_strength": 0.3,  # оставить как есть
    }
    condition_ema_threshold = 0.3  # Порог для условий EMA

    # Настройки трейлинг-стопа
    trailing_stop = True
    trailing_stop_positive = 0.002  # Уменьшен для более быстрой фиксации прибыли
    trailing_stop_positive_offset = 0.0025  # Уменьшен для более быстрой фиксации прибыли
    ...
    min_dca_time = 1  # Уменьшен для более частых докупок

    # Параметры для управления рисками
    risk_per_trade = 0.015  # 1.5% риска на сделку (уменьшен для более консервативной торговли)
    atr_period = 14  # Период для расчета ATR
    atr_multiplier = 1.8  # Множитель для расчета стоп-лосса на основе ATR (уменьшен для более консервативной торговли)
    ...
    _pair_specific_thresholds = {
        "BTC/USDT": {
            "normal_buy_imbalance_threshold": 2.2,  # Уменьшен для более частых входов
            "normal_buy_volume_threshold": 0.25,  # Уменьшен для более частых входов
            "normal_buy_rsi_threshold": 42,  # Уменьшен для более ранних входов
            "cooldown_period": 30,  # Уменьшен для более частой торговли
        },
        "ETH/USDT": {
            "normal_buy_imbalance_threshold": 1.8,  # Уменьшен для более частых входов
            "normal_buy_volume_threshold": 0.15,  # Уменьшен для более частых входов
            "normal_buy_rsi_threshold": 40,  # Уменьшен для более ранних входов
            "cooldown_period": 15,  # Уменьшен для более частой торговли
        },
        "BNB/USDT": {
            "normal_buy_imbalance_threshold": 2.5,  # Уменьшен для более частых входов
            "normal_buy_volume_threshold": 0.2,  # Уменьшен для более частых входов
            "normal_buy_rsi_threshold": 37,  # Уменьшен для более ранних входов
            "cooldown_period": 20,  # Уменьшен для более частой торговли
        },
        "DOT/USDT": {
            "normal_buy_imbalance_threshold": 1.8,  # Уменьшен для более частых входов
            "normal_buy_volume_threshold": 0.15,  # Уменьшен для более частых входов
            "normal_buy_rsi_threshold": 40,  # Уменьшен для более ранних входов
            "cooldown_period": 10,  # Уменьшен для более частой торговли
        },
        "SOL/USDT": {
            "normal_buy_imbalance_threshold": 1.8,  # Уменьшен для более частых входов
            "normal_buy_volume_threshold": 0.15,  # Уменьшен для более частых входов
