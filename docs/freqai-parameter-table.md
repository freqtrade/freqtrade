...
| `plot_feature_importances` | Create a feature importance plot for each model for the top/bottom `plot_feature_importances` number of features. Plot is stored in `user_data/models/identifier/sub-train-COIN_<timestamp>`. <br> **Datatype:** Integer. <br> Default: `0`.
