...
| `label_period_candles` | Number of candles into the future that the labels are created for. This is used in `set_freqai_targets()` (see `templates/FreqaiExampleStrategy.py` for detailed usage). You can create custom labels and choose whether to make use of this parameter or not. <br> **Datatype:** Positive integer.
