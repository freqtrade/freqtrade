...
from freqtrade.exceptions import ExchangeError, PricingError, StrategyError
...
class PlaybackSession:
    ...
    def get_slice(self, limit: int | None = None) -> DataFrame:
    ...
    def get_annotations(self, dataframe: DataFrame) -> list[AnnotationType]:
...
class RPC:
    ...
    def _rpc_playback_init(
        ...
        if not pair or not timeframe:
            raise RPCException("Pair and timeframe are required to initialize playback.")

        timerange_parsed = TimeRange.parse_timerange(timerange or self._config.get("timerange"))
        strategy = self._freqtrade.strategy
        strategy_name = strategy.get_strategy_name() if strategy else ""
        startup_candles = strategy.startup_candle_count if strategy else 0
        candle_type = candle_type or self._config.get("candle_type_def", CandleType.SPOT)
        ...
        if pair not in _data:
            raise RPCException(
                f"No data for {pair}, {timeframe} in {timerange_parsed.timerange_str} found."
            )
        ...
        return self._rpc_playback_state(limit=limit, selected_cols=selected_cols)

    def _rpc_playback_control(
        self,
        action: str,
        ...
        if action == "play":
            ...
            session.controller.set_step(step)
        else:
            raise RPCException("Invalid playback action.")

        if (
            action == "speed"
            and session.last_data is not None
            and session.last_limit == limit
            and session.last_selected_cols == selected_cols
        ):
            return {
    ...
    def _rpc_playback_state(
        ...
        data_slice = session.get_slice(limit)
        annotations = session.get_annotations(data_slice)
        data = RPC._convert_dataframe_to_dict(
        ...
        session.last_selected_cols = selected_cols
        return {
            "state": session.controller.get_state(),
            "data": data,
        }
