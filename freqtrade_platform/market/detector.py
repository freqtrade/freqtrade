...
class MarketRegimeDetector:
    ...
    def _timeframe_features(self, observations: list[MarketObservation]) -> dict[str, dict[str, float]]:
        features_by_timeframe: dict[str, dict[str, float]] = {}
        for observation in observations:
            if not isinstance(observation.metadata, dict):
                continue
            ...
            if not isinstance(obs_features, dict):
                continue
            features_by_timeframe[observation.timeframe] = {
                str(key): float(value) for key, value in obs_features.items()
            }
    ...
    def detect(self, observations: list[MarketObservation] | tuple[MarketObservation, ...]) -> MarketRegimeResult:
        ...
        latest = ordered[-1]
        timeframe_features = self._timeframe_features(ordered)
        latest_features = timeframe_features.get(latest.timeframe, {})
        ...
        history_length = float(latest_features.get("history_length", 0.0))
        if history_length < 3.0:
            ...
            return MarketRegimeResult(
                ...
                metadata={"source": "platform_market_regime_detector", "quality": "insufficient_history", "timeframe_alignment": alignment_score},
            )

        trend = float(latest_features.get("trend_strength", 0.0))
        directional_strength = float(latest_features.get("directional_strength", 0.0))
        volatility = float(latest_features.get("volatility", 0.0))
        atr = float(latest_features.get("normalized_atr", 0.0))
        breakout = float(latest_features.get("breakout_bias", 0.0))
        momentum = float(latest_features.get("momentum", 0.0))
        range_width = float(latest_features.get("range", 0.0))
        structure = float(latest_features.get("structure", 0.0))
        ...
        elif 0.02 <= abs(trend) <= 0.08 and abs(momentum) > 2.0 and abs(breakout) < 0.08 and structure > 0.08:
            regime = MarketRegimeType.TRANSITION
            dominant_signal = "structure"
            confidence = max(0.4, min(0.85, 0.45 + (1.0 - alignment_score) * 0.35 + min(0.2, volatility / 30.0)))
        ...
        elif atr > 4.0 and abs(breakout) < 0.02 and volatility > 5.0 and volatility <= 12.0 and atr <= 8.0:
            regime = MarketRegimeType.VOLATILE_RANGE
            dominant_signal = "range"
            confidence = max(0.45, min(0.95, 0.5 + min(0.3, volatility / 25.0) + alignment_score * 0.2))
        elif (atr > 8.0 or volatility > 12.0) and abs(trend) <= 0.08 and abs(breakout) < 0.08:
